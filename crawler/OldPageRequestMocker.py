...
class RequestWrapper(object):
    ...
    def get_request(self):
        if not self.mock_request_status:
...
if __name__ == "__main__":
    ...
    real_request = RequestWrapper(real_site, real_site_test_status).get_request()
    print (real_request.text[:500])

    test_site = 'aboutus.html'
    test_site_test_status = True
    test_request = RequestWrapper(test_site, test_site_test_status).get_request()
